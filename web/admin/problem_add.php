...
$hint = RemoveXSS($hint);
//echo "->".$OJ_DATA."<-"; 
$pid = addproblem($title, $time_limit, $memory_limit, $description, $input, $output, $sample_input[0], $sample_output[0], $hint, $source, $spj, $OJ_DATA);
$basedir = "$OJ_DATA/$pid";
mkdir($basedir);
...
$sample_basedir = "$OJ_SAMPLE_DATA/$pid";//题目样例数据文件夹
mkdir($sample_basedir);//加载样例数据文件夹
$len = count($sample_input);
for($i = 0; $i < $len; $i++){
  if(strlen($sample_output[$i]) && !strlen($sample_input[$i])) $sample_input[$i] = "0";
  if(strlen($sample_input[$i])) mkdata($pid, "sample".($i + 1).".in", $sample_input[$i], $OJ_SAMPLE_DATA);
  if(strlen($sample_output[$i])) mkdata($pid, "sample".($i + 1).".out", $sample_output[$i], $OJ_SAMPLE_DATA);
}
